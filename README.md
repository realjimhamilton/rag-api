...
- `HYBRID_SEARCH_ENABLED`: (Optional, `VECTOR_DB_TYPE=pgvector` only) Master switch for hybrid retrieval, which fuses Postgres full-text (lexical) search with the pgvector cosine search so exact keyword / template / brand / filename matches that dense embeddings under-rank still surface. Default `False`. The `/query` and `/query_multiple` endpoints also accept a per-request `hybrid` boolean that overrides this switch (so a caller can A/B by user without a second deployment). When enabled, a keyword-matched chunk gets a rank-based bonus added on top of its cosine relevance; a chunk with no keyword match scores exactly as it would under pure vector search, so non-keyword queries are unchanged. Requires the `idx_langchain_pg_embedding_document_fts` GIN index, which is created idempotently at startup.
- `HYBRID_FTS_LANGUAGE`: (Optional) Postgres text-search config (regconfig) used for `to_tsvector` and `websearch_to_tsquery` in hybrid search. Default `english` (stemming + stopword removal, good general recall); `simple` favors exact-token matching. Must be a bare identifier (it is interpolated as a SQL literal and must match the FTS index expression). Changing it after the index is built requires dropping and recreating `idx_langchain_pg_embedding_document_fts`.
- `HYBRID_KEYWORD_BONUS`: (Optional) Relevance bonus added for a full-text match, scaled by the chunk's rank among a file's keyword hits (`bonus = HYBRID_KEYWORD_BONUS / position`). Default `0.35`. Added on top of cosine relevance and capped so distances stay non-negative. Higher values make keyword matches outrank semantically-closer non-keyword chunks more aggressively.
